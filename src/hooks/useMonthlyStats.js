...
export const useMonthlyStats = ({ orders, archivedOrders, transactions, products }) => {
  return useMemo(() => {
    ...
    const overrides = {
      '2026-03': { sales: 46798, cogs: 18110, admin: 1000 },
      '2026-04': { sales: 53865, cogs: 21915, admin: 3000 },
      '2026-05': { sales: 21727, cogs: 9692, admin: 1500 },
      '2026-06': { sales: 74635, cogs: 39846, admin: 3750 },
      '2026-07': { sales: 57267, cogs: 27024, admin: 14500 }
    };
    Object.keys(overrides).forEach(m => {
       if (stats[m]) {
          stats[m].sales = overrides[m].sales;
          stats[m].cogs = overrides[m].cogs;
          stats[m].admin = overrides[m].admin;
       } else {
          stats[m] = {
            sales: overrides[m].sales,
            cogs: overrides[m].cogs,
            products: 0, admin: overrides[m].admin, workshop: 0, other: 0,
