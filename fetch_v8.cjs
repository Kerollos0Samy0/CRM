...
async function run() {
  ...
  json.table.rows.forEach(row => {
      ...
      const church = obj['col_4'] || ''; // col_4 is Church!
      const key = clientName + '_' + church;
      
      updates[key] = { status };
  });
  
  fs.writeFileSync('src/data/orders_status_update_v8.json', JSON.stringify(updates, null, 2));
  console.log('Saved to orders_status_update_v8.json, count:', Object.keys(updates).length);
}
